...
- **Subscribe-spec contention** resolved: each subscribe/unsubscribe spec uses a distinct seeded popular tag (typescript / python / ai-prompts / react / devops — verified against `scripts/seed.sql` lines 27–31) so cross-spec interference is impossible at workers=4.
...
**Seeded tags (verified against `scripts/seed.sql` lines 27–31):** `typescript`, `python`, `ai-prompts`, `react`, `devops`. All five have `post_count > 0` and are eligible for the popular-tags widget. The amendment uses these names verbatim — no invented tag names.
...
3. `subscribed-tag-appears-in-following.spec.ts` — pre-state: testuser pre-subscribed to `python` via API in beforeEach; visit `/`; assert sidebar Following list contains `subscribed-tag-link-python`. **Tag: python.** (Distinct from #2's tag to eliminate per-row contention under workers=4.)
4. `unsubscribe-from-sidebar.spec.ts` — pre-state: testuser pre-subscribed to `ai-prompts` via API in beforeEach; click `subscribe-btn-ai-prompts` (its label flipped to "Unsubscribe"); assert `aria-pressed="false"` after click. **Tag: ai-prompts.**
5. `subscribed-tag-feed.spec.ts` — pre-state: testuser pre-subscribed to `react` via API in beforeEach; visit `/following`; assert ≥ 1 post visible AND each visible `post-list-item` has `react` among its tags. **Tag: react.** (Seed has 2 react-tagged posts: `c0000000-...-000000000004` and `c0000000-...-000000000011`.)
6. `tag-page.spec.ts` — visit `/tags/typescript`; assert `tag-page` rendered, `tag-page-title` text contains `typescript`, post list has ≥ 1 item. (Render checklist.) **Tag: typescript** — read-only, does not write subscription state, so reuse with #2 is safe.
7. `my-subscriptions-list.spec.ts` — log in as alice (seeded with sub to tag `b0000000-...-000000000003` = `ai-prompts`); assert sidebar Following list shows `subscribed-tag-link-ai-prompts`. (Render checklist.) **User: alice, tag: ai-prompts.** Distinct user from testuser-using specs, so no contention.
8. `click-tag-from-post.spec.ts` — visit a seeded post tagged `typescript` (e.g., `c0000000-...-000000000001`); click `post-tag-chip-typescript`; assert URL is `/tags/typescript`. (Read-only navigation; no subscription writes.)
9. `subscribe-from-tag-page.spec.ts` — visit `/tags/devops` as testuser (no pre-state); click `subscribe-btn-devops`; assert `aria-pressed="true"`. **Tag: devops.**

**Subscribe-spec contention resolution:** the four specs that WRITE subscription state for testuser use four distinct seeded tags: `typescript` (#2), `python` (#3), `ai-prompts` (#4), `devops` (#9). Spec #5 also writes for testuser but uses `react`, the fifth distinct tag. Read-only specs (#1, #6, #7, #8) do not write subscription state. Across workers running as testuser, each write spec touches a different `(testuser, tag_id)` row; the PK on `user_tag_subscriptions` makes per-row writes conflict-free. The reset fixture (`fixtures/reset.ts`) clears testuser's `user_tag_subscriptions` rows between specs (per the parent design's reset pattern), so each spec's beforeEach establishes its own precondition cleanly. **Open question for the implementer: confirm `fixtures/reset.ts` is per-spec scope, NOT per-worker shared scope** — if reset somehow runs only once per worker, two writing specs running in the same worker could see each other's state. The parent design implies per-spec reset, but verify before relying on it.
...
| Cross-spec contention on shared tag rows in subscribe specs                               | Distinct seeded popular tags per spec (typescript / python / ai-prompts / react / devops — all verified seeded). PK on `user_tag_subscriptions` makes per-(user,tag) row updates conflict-free. Reset fixture zeroes testuser subs between specs. |
...
- [ ] Subscribe specs use distinct seeded tags (typescript / python / ai-prompts / react / devops — all in `scripts/seed.sql` lines 27–31; no invented tag names).
