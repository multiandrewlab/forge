...
test('search: Cmd+K (or Ctrl+K) opens the search modal', async ({ testuser }) => {
  await testuser.goto('/');
  ...
  await search.searchTrigger(testuser).focus();
  ...
  const isMac = await testuser.evaluate(() => navigator.platform.toLowerCase().includes('mac'));
  await testuser.keyboard.press(isMac ? 'Meta+K' : 'Control+K');
