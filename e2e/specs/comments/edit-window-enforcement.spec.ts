...
test.fixme('comments: cannot edit own comment after the edit window expires', async ({ actor }) => {
  ...
  const c = await actor.request.post(`/api/posts/${postId}/comments`, {
    headers: auth,
    data: { body: 'too late' },
  });
  ...
  await actor.clock.install({ time: new Date('2026-04-30T12:00:00Z') });
  await actor.goto(`/posts/${postId}`);

  // Advance N+1 minutes past whatever window the server enforces
  await actor.clock.fastForward('20:00'); // 20 minutes
