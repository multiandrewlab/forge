...
test('comments: GET /comments returns 404 after the post is deleted', async ({ actor }) => {
  ...
  const del = await actor.request.delete(`/api/posts/${postId}`, { headers: auth });
  expect(del.ok()).toBe(true);

  // Comments route returns 404
  const after = await actor.request.get(`/api/posts/${postId}/comments`, { headers: auth });
