...
test('voting: clicking upvote twice removes the vote (idempotency) on a fresh post', async ({
  actor,
}) => {
  ...
  const created = await actor.request.post('/api/posts', {
    ...
    data: {
      title: `Idempotency seed ${Date.now()}`,
      contentType: 'snippet',
      language: 'typescript',
      content: 'x',
      visibility: 'public',
  ...
  await expect(voting.voteScore(actor)).toHaveText('0');
