...
test('post view: navigating to a missing post id shows "Post not found"', async ({ actor }) => {
  await actor.goto(`/posts/${MISSING_POST_ID}`);
  ...
  await expect(actor.locator('div.text-gray-400.text-center.py-12')).toHaveText('Post not found');
});
