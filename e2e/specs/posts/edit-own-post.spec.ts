...
test('edit: own post saves changes and the title updates', async ({ actor }) => {
  ...
  await posts.newPostTitle(actor).fill(newTitle);
  ...
  await expect(posts.newPostTitle(actor)).toHaveValue(newTitle);
});
