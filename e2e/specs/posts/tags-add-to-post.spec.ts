...
test('tags: typing a tag and pressing Enter adds it as a chip in the editor', async ({ actor }) => {
  ...
  await expect(actor.getByTestId('tag-item').filter({ hasText: 'react' })).toBeVisible();
});
