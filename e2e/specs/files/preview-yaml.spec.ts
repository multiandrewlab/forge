...
const FIX = join(__dirname, '..', '..', 'fixtures', 'files', 'sample.yaml');

test('preview yaml: highlighted code variant renders the marker key', async ({
  actor,
}, testInfo) => {
  const stamp = `${testInfo.title.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-${Date.now()}`;
  const title = `yaml-${stamp}`;
  ...
  await files.fileSidebarItem(actor, 'sample.yaml').click();
  await expect(files.filePreviewCode(actor)).toContainText('e2e-yaml-fixture');
});
