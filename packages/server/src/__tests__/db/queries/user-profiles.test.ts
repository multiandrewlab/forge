...
describe('user-profiles queries', () => {
  ...
  describe('getUserPostCount', () => {
    it('returns the count of public non-draft non-deleted posts', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('COUNT(*)'), [userId]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining("visibility = 'public'"), [
        userId,
      ]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('is_draft = false'), [userId]);
  ...
  describe('getUserTotalVotes', () => {
    it('returns the COALESCE sum of vote_count across non-deleted posts', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('COALESCE'), [userId]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('vote_count'), [userId]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('deleted_at IS NULL'), [
        userId,
      ]);
  ...
  describe('getUserTopTags', () => {
    it('returns top 5 tags by vote sum with JOINs through post_tags', async () => {
      ...
      mockQuery.mockResolvedValue({
        rows: rows.map((r) => ({ ...r, vote_sum: String(r.vote_sum) })),
      });
      const result = await getUserTopTags(userId);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('post_tags'), [userId]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('tags'), [userId]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('LIMIT 5'), [userId]);
  ...
  describe('getTagExperts', () => {
    it('returns tags where user is top contributor with CTE and HAVING thresholds', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('DISTINCT ON'), [userId]);
      // Verify minimum thresholds
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('HAVING'), [userId]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('post_tags'), [userId]);
      expect(result).toEqual(rows);
    });
  ...
  describe('getUserPublicPosts', () => {
    it('returns paginated public posts with STRING_AGG tags', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('STRING_AGG'), [userId, 10]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining("visibility = 'public'"), [
        userId,
        10,
      ]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('is_draft = false'), [
        userId,
        10,
      ]);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('deleted_at IS NULL'), [
